#!/usr/bin/env python
# -*- coding: UTF-8 -*-
# coding=utf-8 

"""
@author: Li Tian
@contact: litian_cup@163.com
@software: pycharm
@file: tk_app.py
@time: 2021/1/15 15:45
@desc: 可视化界面
"""
from dataclasses import dataclass
from tkinter import *
from tkinter import ttk
from point_test.properties import *
from system_hotkey import SystemHotkey
from point_test.mouse_action import MouseAction
from threading import Thread
from time import sleep


@dataclass
class App:

    def __post_init__(self):

        self.font1 = ('微软雅黑', 16)
        self.font_normal = ('微软雅黑', 14)

        self.root = Tk()
        self.root.title('狗贼v0.1')

        self.frame_1 = Frame(self.root)

        # 当前配置文件中的字典
        self.current_dict: dict = {}
        # 上述字典中的某个值
        # self.current_key: str = ''
        self.cmb1_value = StringVar()
        self.cmb2_value = StringVar()
        # 两个entry的值
        self.xy_value = StringVar()
        self.color_value = StringVar()
        # 全局快捷键设置
        self.xy, self.color = '', ''
        self.ma = MouseAction()
        self.hk = SystemHotkey()
        self.hk.register(('alt', '1'), callback=lambda e: self.set_return())

        self.l_title = Label(self.root, text='按【ALT+1】截取坐标和颜色', fg='blue', font=self.font1)
        self.l_title.pack()

        self.l1 = Label(self.frame_1, text='选取功能和按键', font=self.font_normal)
        self.l1.pack(side=LEFT, fill=Y)

        self.cmb1 = ttk.Combobox(self.frame_1, textvariable=self.cmb1_value, width=30, font=self.font_normal)
        self.cmb1['values'] = ['【1】双人御魂（一个电脑）', '【2】待补充']
        # 选择第一个为默认
        # self.cmb1.current(0)
        self.cmb1.bind('<<ComboboxSelected>>', self.get_list)
        self.cmb1.pack(side=LEFT, fill=Y)

        self.cmb2 = ttk.Combobox(self.frame_1, textvariable=self.cmb2_value, font=self.font_normal)
        self.cmb2.bind('<<ComboboxSelected>>', self.get_key)
        self.cmb2.pack(side=LEFT, fill=Y)

        self.b_reset = Button(self.frame_1, text='关闭', font=self.font_normal, command=lambda: self.root.quit())
        self.b_reset.pack(side=LEFT, fill=BOTH, expand=YES)

        self.frame_xyc = Frame(self.root)

        self.l_xy = Label(self.frame_xyc, text='坐标', font=self.font_normal)
        self.l_xy.pack(side=LEFT, fill=Y)

        self.e_xy = Entry(self.frame_xyc, textvariable=self.xy_value, font=self.font_normal)
        self.e_xy.insert(END, self.xy)
        self.e_xy.pack(side=LEFT, fill=Y)

        self.l_c = Label(self.frame_xyc, text='颜色', font=self.font_normal)
        self.l_c.pack(side=LEFT, fill=Y)

        self.e_color = Entry(self.frame_xyc, textvariable=self.color_value, font=self.font_normal)
        self.e_xy.insert(END, self.color)
        self.e_color.pack(side=LEFT, fill=Y)

        self.b1 = Button(self.frame_xyc, text='写入', command=self.write_dict, font=self.font_normal)
        self.b1.pack(side=LEFT, fill=BOTH, expand=YES)

        self.b2 = Button(self.frame_xyc, text='运行', font=self.font_normal)
        self.b2.pack(side=LEFT, fill=BOTH, expand=YES)

        self.frame_1.pack(side=TOP, fill=BOTH)
        self.frame_xyc.pack(side=BOTTOM, fill=BOTH)

    def run(self):
        self.check_mouse_move()
        self.root.mainloop()

    def get_list(self, *args):
        """获取对应下拉框的list，方便写入后续下拉框"""
        check_flag = self.cmb1_value.get()[1]

        if int(check_flag) == 1:
            # 双人御魂（一个电脑）
            self.current_dict = couple_yuhun_single_computer

        self.cmb2['values'] = self.current_dict['names']

    def get_key(self, *args):
        """获取当前的key值"""
        # return self.cmb2_value.get()
        ...

    def write_dict(self):
        """将信息写入到dict中"""
        aim_key = self.cmb2_value.get()
        aim_value = [self.xy, self.color]
        if aim_key != '':
            self.current_dict[aim_key] = aim_value
        print(self.current_dict)

    def set_return(self):
        """设置绑定后的按键操作的值"""
        self.xy, self.color = self.ma.get_mouse_postion_color().get_info()
        self.e_xy.delete(0, END)
        self.e_xy.insert(END, str(self.xy))
        self.e_color.delete(0, END)
        self.e_color.insert(END, str(self.color))

    def check_mouse(self):
        """循环检测鼠标"""
        while True:
            self.set_return()
            sleep(0.01)

    def check_mouse_move(self):
        """检测鼠标移动，实时显示鼠标位置和颜色"""
        t = Thread(target=self.check_mouse)
        t.start()

    def destroy(self):
        self.root.quit()
        ...



if __name__ == '__main__':
    App().run()
